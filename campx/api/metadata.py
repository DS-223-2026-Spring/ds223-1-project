...
API_ASSUMPTIONS = (
    ...
    "Simulation creation is exposed through `POST /simulations` as the main public write route for simulation records and currently launches an in-process background run.",
    "`POST /simulations/{simulation_id}/step` exposes the same t -> t+1 LinUCB decision, latent-aware outcome, and model-update loop for orchestrated single-step execution.",
    "The `decide` endpoint now computes LinUCB-style exploit and explore scores through the shared inference routing used by simulation stepping.",
