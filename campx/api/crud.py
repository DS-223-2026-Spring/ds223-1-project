...
from models import CUSTOMER_FIELDS, LATENT_FIELDS
from schema import CustomerCreate, CustomerUpdate, DecideRequest, FeedbackRequest, SimulationCreate
...
def update_customer_record(
    ...
    if customer_fields:
        ...
        query = sql.SQL(
            "UPDATE public.customers SET {} WHERE customer_id = %s"
    ...
    if latents:
        ...
        columns = [sql.Identifier("customer_id")] + [
            sql.Identifier(field) for field in latent_fields
        ]
        placeholders = sql.SQL(", ").join(
            [sql.Placeholder()] * (len(latent_fields) + 1)
        )
        ...
        db.cursor.execute(
            query,
            [customer_id] + [latents[field] for field in latent_fields],
        )
...
def submit_feedback(db: SQLHandler, payload: FeedbackRequest) -> dict[str, Any] | None:
    ...
    observed_at = data.get("observed_at") or datetime.utcnow()
