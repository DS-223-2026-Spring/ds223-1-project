...
st.caption("Browse the synthetic customer pool, filter by segment and RFM features, and inspect interaction history.")
...
if customers.empty:
    st.info("No customers in the database yet. Run customer generation or load seed data.")
    st.stop()
...
with st.expander("How customer segments are defined"):
...
with f1:
    segments = sorted(customers["segment_label"].dropna().unique().tolist()) if "segment_label" in customers else []
    selected_segments = st.multiselect("Segment", options=segments, default=segments)
with f2:
    genders = sorted(customers["gender"].dropna().unique().tolist()) if "gender" in customers else []
    selected_genders = st.multiselect("Gender", options=genders, default=genders)
with f3:
    name_search = st.text_input("Customer ID search", placeholder="exact ID, e.g. 42")
...
with f4:
    if "recency" in customers:
        ...
        rec_range = st.slider("Recency range", min_value=rec_min, max_value=rec_max, value=(rec_min, rec_max))
    else:
        rec_range = None
with f5:
    if "monetary" in customers:
...
if rec_range and "recency" in filtered:
    filtered = filtered[(filtered["recency"] >= rec_range[0]) & (filtered["recency"] <= rec_range[1])]
if mon_range and "monetary" in filtered:
    filtered = filtered[(filtered["monetary"] >= mon_range[0]) & (filtered["monetary"] <= mon_range[1])]
...
if "segment_label" in filtered and not filtered.empty:
    ...
    seg_counts = filtered["segment_label"].value_counts().rename_axis("Segment").to_frame("Customers")
    st.bar_chart(seg_counts, height=260, color="#0f766e")
    st.write("")
...
display_cols = [c for c in [
    "customer_id", "segment_label", "gender", "recency", "frequency", "monetary",
...
with selector_col:
    selected_id = st.selectbox("Pick a customer", options=filtered["customer_id"].tolist(), format_func=lambda x: f"#{x}")
with debug_col:
    show_latents = st.toggle(
        "Show simulation-only debug variables",
        value=False,
        help="For development/demo only. These generated variables are not customer-facing decision inputs.",
    )
...
if rfm:
    ...
    st.dataframe(pd.DataFrame([rfm]), hide_index=True, width="stretch")

if show_latents and "latents" in detail and detail["latents"]:
    with st.expander("Simulation-only debug variables"):
...
if interactions:
    ...
    if "decision_at" in inter_df:
        inter_df["decision_at"] = pd.to_datetime(inter_df["decision_at"], errors="coerce", utc=True).dt.tz_localize(None).dt.strftime("%Y-%m-%d %H:%M")
    if "action" in inter_df:
        inter_df["action"] = inter_df["action"].map(lambda a: bu.ACTION_LABELS.get(a, a))
    if "revenue" in inter_df:
        inter_df["revenue"] = inter_df["revenue"].apply(lambda x: f"£{x:.2f}" if pd.notna(x) else "—")
    if "reward" in inter_df:
        inter_df["reward"] = inter_df["reward"].apply(lambda x: f"£{x:+.2f}" if pd.notna(x) else "—")
    show_cols = [c for c in ["interaction_id", "simulation_id", "decision_at", "action", "converted", "revenue", "reward"] if c in inter_df.columns]
